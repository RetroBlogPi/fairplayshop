	...
	public function displayForm()
	{
		...
		'.($advert->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$advert->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/adverts.gif" />'.$this->l('Advert').'</legend>
				...
				<div class="margin-form">
					<textarea rows="3" cols="33" name="name" />'.htmlentities(Tools::getValue('name', $advert->name), ENT_COMPAT, 'UTF-8').'</textarea><sup> *</sup>
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
				...
				foreach ($languages as $language)
					...
					<div id="description_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
						<textarea rows="3" cols="33" name="description_'.$language['id_lang'].'"/>'. htmlentities($this->getFieldValue($advert, 'description', intval($language['id_lang'])), ENT_COMPAT, 'UTF-8').'</textarea><sup> *</sup>
						<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
						<p></p>
					</div>';							
