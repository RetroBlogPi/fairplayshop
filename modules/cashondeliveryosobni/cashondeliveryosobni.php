...
*/
class CashOnDeliveryOsobni extends PaymentModule
{	
 	private	$_html = '';
	...
	{
		$this->name = 'cashondeliveryosobni';
		$this->tab = 'Payment';
		$this->version = 1.0;
	...
	function install()
	{
		if (!parent::install() OR !Configuration::updateValue('COD_FEE2', '0') OR !Configuration::updateValue('COD_FEE2_TYPE', '0') OR !Configuration::updateValue('COD_FEE2_MIN', '0')
	...
	{
		if (!Configuration::deleteByName('COD_FEE2') OR (!Configuration::deleteByName('COD_FEE2_TYPE')) OR !parent::uninstall() OR (!Configuration::deleteByName('COD_FEE2_MIN')))
			return false;
		return true;
	}
	...
	{
		$this->_html = '<h2>Platba hotově</h2>';
		if (isset($_POST['submitCOD']))
		{
			...
			{
				Configuration::updateValue('COD_FEE2', floatval( $_POST['fee']) );
				Configuration::updateValue('COD_FEE2_TYPE', floatval( $_POST['feetype']) );
				Configuration::updateValue('COD_FEE2_MIN', floatval( $_POST['feemin']) );
				$this->displayConf();
			}
			else
				$this->displayErrors();
	...
	public function displayCOD()
	{
		...
		<b>'.$this->l('Tento modul umožňuje nastavit poplatek za platbu v hotovosti.').'</b><br /><br /><br />';
	}
	
	
	public function displayFormSettings()
	{
		$conf = Configuration::getMultiple(array('COD_FEE2','COD_FEE2_TYPE','COD_FEE2_MIN'));
		$fee = array_key_exists('fee', $_POST) ? $_POST['fee'] : (array_key_exists('COD_FEE2', $conf) ? $conf['COD_FEE2'] : '');
		$feetype = array_key_exists('feetype', $_POST) ? $_POST['feetype'] : (array_key_exists('COD_FEE2_TYPE', $conf) ? $conf['COD_FEE2_TYPE'] : '');
		$feemin = array_key_exists('feemin', $_POST) ? $_POST['feemin'] : (array_key_exists('COD_FEE2_MIN', $conf) ? $conf['COD_FEE2_MIN'] : '');
...
public function displayConf()
	...
	{
	if(Configuration::get('COD_FEE2_TYPE')==0)
	{
		return floatval(Configuration::get('COD_FEE2'));
	}
	else
	{
		$minimalfee = floatval(Configuration::get('COD_FEE2_MIN'));
		$cartvalue = floatval($params['cart']->getOrderTotal(true, 3));
		$percent = floatval(Configuration::get('COD_FEE2'));
		$percent = $percent / 100;
	...
	function validateOrderCOD($id_cart, $id_order_state, $amountPaid, $paymentMethod = 'Unknown', $message = NULL, $extraVars = array(), $currency_special = NULL)
	{
        ...
        $message_email = "";
            ...
            // Povrzeni objednavky vlastnikovi                              						
						Mail::Send(intval($order->id_lang), 'order_conf', 'Vase objednavka c.: '.$order->id, $data, Configuration::get('PS_SHOP_EMAIL'), '', $customer->email, $customer->firstname.' '.$customer->lastname, NULL);					
