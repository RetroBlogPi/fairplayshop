...
<div id="new-products_block_right" class="block products_block">
	...
	{if $new_products|@count > 0}
		<ul class="product_images">
			<li><a href="{$new_products.0.link}" title="{$new_products.0.legend}"><img src="{$link->getImageLink($new_products.0.link_rewrite, $new_products.0.id_image, 'medium')}" alt="{$new_products.0.legend}" /></a></li>
			{if $new_products|@count > 1}<li><a href="{$new_products.1.link}" title="{$new_products.1.legend}"><img src="{$link->getImageLink($new_products.1.link_rewrite, $new_products.1.id_image, 'medium')}" height="{$mediumSize.height}" width="{$mediumSize.width}" alt="{$new_products.1.legend}" /></a></li>{/if}
		</ul>
		<dl class="products">
		{foreach from=$new_products item=newproduct name=myLoop}
			<dt class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}"><a href="{$newproduct.link}" title="{$newproduct.name}">{$newproduct.name|strip_tags}</a></dt>
