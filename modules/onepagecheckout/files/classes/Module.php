...
abstract class Module
{
	...
	public static function hookExecPaymentAjax($id_country,$id_carrier=0,$ship2pay=false)
	{
		global $cart, $cookie;
		...
		if ($ship2pay) {
		  ...
		  if(count($result)>0) {
		    $ship2payField = 'stp.id_carrier,';
		    $ship2payJoin = 'LEFT JOIN `'._DB_PREFIX_.'shiptopay` stp ON hm.`id_module` = stp.`id_payment`';
		    $ship2payWhere = 'AND stp.id_carrier='.intval($id_carrier);
		  }		
		}//if($ship2pay)

		# get first (any) customer and (any) address, in address, change id_country
		$any_customer = Db::getInstance()->getRow('SELECT id_customer FROM `'._DB_PREFIX_.'customer`');
		$any_address = Db::getInstance()->getRow('SELECT id_address FROM `'._DB_PREFIX_.'address`');
	

		$cart_arg = $cart;
		$cart_arg->id_customer = $any_customer['id_customer'];
		$cart_arg->id_address_invoice = $any_address['id_address'];
		$cart_arg->id_address_delivery = $any_address['id_address'];
		
		$hookArgs = array('cookie' => $cookie, 'cart' => $cart_arg);
		$output = '';

		// tu musime injectnut address a customer namiesto $cart->id_address_invoice a $cart->id_customer
		// pri requeste by teoreticky slo pouzit nejakeho existujuceho zakaznika a adresu (na frontende sa to aj tak nezobrazi)
		// potom po potrvrdeni objednavky by sa zavolal processCarrier ako teraz ale ajaxom na pozadi, refreshli by sa payment methods
		// tentokrat s realnymi datami a "klikol" by sa link vybratej payment metody		


		$result = Db::getInstance()->ExecuteS('
		SELECT DISTINCT '.$ship2payField.' h.`id_hook`, m.`name`, hm.`position`
		FROM `'._DB_PREFIX_.'module_country` mc
		LEFT JOIN `'._DB_PREFIX_.'module` m ON m.`id_module` = mc.`id_module`
		INNER JOIN `'._DB_PREFIX_.'module_group` mg ON (m.`id_module` = mg.`id_module`)
		LEFT JOIN `'._DB_PREFIX_.'hook_module` hm ON hm.`id_module` = m.`id_module`
		...
		WHERE h.`name` = \'payment\'
		'.$ship2payWhere.'
		AND mc.id_country = '.intval($id_country).'
		AND m.`active` = 1
		ORDER BY hm.`position`, m.`name` DESC');
		if ($result)
			foreach ($result AS $k => $module)
