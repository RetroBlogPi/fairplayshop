	...
	<td class="cart_availability">
		{if $product.active AND ($product.allow_oosp OR $product.stock_quantity > 0)}
			<img src="{$img_dir}icon/available.gif" alt="{l s='Available' template='shopping-cart-product-line'}" />
		{else}
	...
	<td class="cart_quantity">
		...
		<div  style="float: left; padding-top: 0px; padding-left: 15px;">{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.quantity-$quantityDisplayed}{/if}</div>
