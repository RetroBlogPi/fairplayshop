...
<h2 style="margin-top: 0px"><div style="float: right; margin-right: 18px;"><span style="text-transform: none;">{l s='Your shopping cart contains' template='shopping-cart'} {$products|@count} {if $products|@count > 1}{l s='products' template='shopping-cart'}{else}{l s='product' template='shopping-cart'}{/if}</span></div>{l s='Shopping cart summary' template='shopping-cart'}</h2>
...
<br />
<b><a href="#" onClick="onepage_cartupdate(document, document.checkoutform, '{if $smarty.server.HTTP_REFERER && strstr($smarty.server.HTTP_REFERER, 'order.php')}{$base_dir_ssl}index.php{else}{$smarty.server.HTTP_REFERER}{/if}');"> &laquo; {l s='Continue shopping' template='shopping-cart'} </a>&nbsp; {l s='or fill in the form below to finish your order.'}</b>
<br />
<br />
...
{if !$isLogged && !$one_page_checkout_settings.hide_email}
                        ...
                        <fieldset id="alr_fieldset" style="padding-bottom: 0px;">
	                        ...
	                        <img id="alr_wait_img" align="right" src="{$img_dir}ajax-loader.gif" style="margin-right: 48%; display: none;position:relative;top:76px;margin-top:-33px;z-index:100;" />
                                <h4>{l s='Pro pøihlášení zadejte Váš e-mail a Vaše heslo' template='authentication'}.</h4>
...
{if !($virtual_cart AND $one_page_checkout_settings.virtual_no_delivery)}
                        <p class="text" {if !$one_page_checkout_settings.company_delivery}style="display:none;"{/if}>
				...
				{if $one_page_checkout_settings.ex_texts}<i class="ex_blur">&nbsp;&nbsp;({l s='ex.'} {l s='Paris'})</i>{/if}
                        ...
                        <p class="text" {if !$one_page_checkout_settings.phone}style="display:none;"{/if}>
