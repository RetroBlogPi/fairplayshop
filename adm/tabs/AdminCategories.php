...
class AdminCategories extends AdminTab
{
	...
	public function display($token = NULL)
	{
		...
		$this->getList(intval($cookie->id_lang), !Tools::getValue($this->table.'Orderby') ? 'sort_order' : NULL, !Tools::getValue($this->table.'Orderway') ? 'ASC' : NULL);
	...
	public function postProcess()
	{
		...
		$this->uploadRootImage();
		
		parent::postProcess();
	}

  public function uploadRootImage() {
    $file=$_FILES['imageroot'];
    if(!$file) return;
    
    //@unlink(_PS_CAT_IMG_DIR_."root/".$this->_category->id.'-root.jpg');
    move_uploaded_file($file['tmp_name'],_PS_CAT_IMG_DIR_."root/".$this->_category->id.'-root.jpg');
  }
    ...
    $this->displayImage($obj->id, _PS_IMG_DIR_.'c/'.$obj->id.'.jpg', 350, NULL, Tools::getAdminToken('AdminCatalog'.intval(Tab::getIdFromClassName('AdminCatalog')).intval($cookie->id_employee)), 'left');
		echo '
				<label>'.$this->l('Image Root:').' </label>
				<div class="margin-form">
					<input type="file" name="imageroot" /></div>';
				if(file_exists(_PS_IMG_DIR_.'c/root/'.$obj->id.'-root.jpg')) echo'<div class="margin-form"><img src="'._THEME_CAT_DIR_.'root/'.$obj->id.'-root.jpg" /></div>';
