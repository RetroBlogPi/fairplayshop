...
class AdminAddresses extends AdminTab
{
	...
	public function displayForm()
	{
		...
		if ($this->addressType != 'manufacturer')
		{
				...
				<label>'.$this->l('IC:').'</label>
				<div class="margin-form">
					<input type="text" size="33" name="ic" value="'.htmlentities($this->getFieldValue($obj, 'ic'), ENT_COMPAT, 'UTF-8').'" />
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
				<label>'.$this->l('DIC:').'</label>
				<div class="margin-form">
					<input type="text" size="33" name="dic" value="'.htmlentities($this->getFieldValue($obj, 'dic'), ENT_COMPAT, 'UTF-8').'" />
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>';
		}
				echo '
