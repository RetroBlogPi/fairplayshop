	...
	public function viewDetails()
	{
		...
		<div style="float: left">
			<fieldset style="width: 400px;">
				...
				'.(!empty($addressDelivery->ic) ? $this->l('IC').':'.$addressDelivery->ic.'<br />' : '').'
				'.(!empty($addressDelivery->dic) ? $this->l('DIC').':'.$addressDelivery->dic.'<br />' : '').'
				'.$addressDelivery->firstname.' '.$addressDelivery->lastname.'<br />
				...
				'.(!empty($addressDelivery->phone) ? $this->l('Tel.').':'.$addressDelivery->phone.'<br />' : '').'
				'.(!empty($addressDelivery->phone_mobile) ? $this->l('Mobil').':'.$addressDelivery->phone_mobile.'<br />' : '').'
		...
		<div style="float: left; margin-left: 40px">
			<fieldset style="width: 400px;">
				...
				'.(!empty($addressInvoice->company) ? $addressInvoice->company.'<br />' : '').'
				'.(!empty($addressInvoice->ic) ? $this->l('IC').':'.$addressInvoice->ic.'<br />' : '').'
				'.(!empty($addressInvoice->dic) ? $this->l('DIC').':'.$addressInvoice->dic.'<br />' : '').'
				'.$addressInvoice->firstname.' '.$addressInvoice->lastname.'<br />
				...
				'.(!empty($addressInvoice->phone) ? $this->l('Tel.').':'.$addressInvoice->phone.'<br />' : '').'
				'.(!empty($addressInvoice->phone_mobile) ? $this->l('Mobil').':'.$addressInvoice->phone_mobile.'<br />' : '').'
							...
							if ($product['product_quantity'] > $product['customizationQuantityTotal'])
							{
								...
								<tr'.((isset($image['id_image']) AND isset($products[$k]['image_size'])) ? ' height="'.($products[$k]['image_size'][1] + 7).'"' : '').'>
									...
									<td><a href="index.php?tab=AdminCatalog&id_product='.$product['product_id'].'&updateproduct&token='.$tokenCatalog.'">
										...
										.(($product['product_reference'] AND $product['product_supplier_reference']) ? ' / '.$product['product_supplier_reference'] : '')
										.'</a></td>
									<td align="center">'.($product['pocet']>0 ? Tools::displayPrice($product['product_price_wt']*100, $currency, false, false) : Tools::displayPrice($product['product_price_wt'], $currency, false, false)).'</td>
									<td align="center" class="productQuantity">'.($product['pocet']>0 ? intval($product['pocet']) : (intval($product['product_quantity']) - $product['customizationQuantityTotal'])).'</td>
