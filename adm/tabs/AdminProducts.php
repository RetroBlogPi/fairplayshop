...
class AdminProducts extends AdminTab
{
	...
	protected function copyFromPost(&$object, $table)
	{
		...
		$object->customsize = (!isset($_POST['customsize']) ? false : true);
	}
			...
			if (Validate::isLoadedObject($product = new Product(intval(Tools::getValue('id_product')))))
			{
				...
				if (!sizeof($this->_errors))
				{
					...
					if (!isset($_POST['attribute_price_impact'])) $_POST['attribute_price_impact'] = 0;
                                        if (!isset($_POST['attribute_price_impact_d1'])) $_POST['attribute_price_impact_d1'] = 0;
                                        if (!isset($_POST['attribute_price_impact_d2'])) $_POST['attribute_price_impact_d2'] = 0;
                                        if (!isset($_POST['attribute_price_impact_d3'])) $_POST['attribute_price_impact_d3'] = 0;
						...
						if ($this->tabAccess['edit'] === '1')
						{
							if ($product->productAttributeExists($_POST['attribute_combinaison_list'], $id_product_attribute))
								...
								Tools::getValue('attribute_price') * Tools::getValue('attribute_price_impact'),
                                                                Tools::getValue('attribute_price_d1') * Tools::getValue('attribute_price_impact_d1'),
                                                                Tools::getValue('attribute_price_d2') * Tools::getValue('attribute_price_impact_d2'),
                                                                Tools::getValue('attribute_price_d3') * Tools::getValue('attribute_price_impact_d3'),
	...
	function displayFormInformations($obj, $currency, $languages, $defaultLanguage)
	{
		...
		echo '
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input type="checkbox" name="customsize" id="customsize" style="padding-top: 5px;" '.($this->getFieldValue($obj, 'customsize') ? 'checked="checked"' : '').'value="1" />&nbsp;<label for="customsize" class="t">'.$this->l('Povolit nákup ve specifické velikosti').'</label>
						</td>
					</tr>
					<tr><td colspan="2"><hr style="width:730px;"></td></tr>';
...
<?php
				...
				echo '</td>
					...
					// wholesale prices added by JM ---------------------------------------------------------------
					echo '<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:764px;" /></td></tr>';
					echo '
					<tr>
						<td class="col-left">'.$this->l('Dealer D1 price:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="priceTE_D1" name="price_D1" type="text" value="'.$this->getFieldValue($obj, 'price_D1').'" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\'); calcPriceTI();" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:2px">'.$this->l('Dealer price D1 without VAT').'</span>
						</td>
					</tr>';
					echo '
					<tr>
						<td class="col-left">'.$this->l('Dealer D2 price:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="priceTE_D2" name="price_D2" type="text" value="'.$this->getFieldValue($obj, 'price_D2').'" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\'); calcPriceTI();" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:2px">'.$this->l('Dealer price D2 without VAT').'</span>
						</td>
					</tr>';
					echo '
					<tr>
						<td class="col-left">'.$this->l('Dealer D3 price:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="priceTE_D3" name="price_D3" type="text" value="'.$this->getFieldValue($obj, 'price_D3').'" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\'); calcPriceTI();" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:2px">'.$this->l('Dealer price D3 without VAT').'</span>
						</td>
					</tr>';
					echo '<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:764px;" /></td></tr>';
					//-----------------------------------------------------------------------------------------							
	...
	function displayFormAttributes($obj, $languages, $defaultLanguage)
	{
		...
		if ($obj->id)
		  ...
		  <tr>
			  <td style="width:150px">'.$this->l('Impact on price D1:').'</td>
			  <td colspan="2" style="padding-bottom:5px;">
				<select name="attribute_price_impact_d1" id="attribute_price_impact_d1" style="width: 140px;" onchange="check_impact();">
				  <option value="0">'.$this->l('None').'</option>
				  <option value="1">'.$this->l('Increase').'</option>
				  <option value="-1">'.$this->l('Reduction').'</option>
				</select> <sup>*</sup>
				<span id="span_impact">&nbsp;&nbsp;'.$this->l('of').'&nbsp;&nbsp;'.($currency->format == 1 ? $currency->sign.' ' : '').'
					<input type="text" size="6" name="attribute_price_d1" id="attribute_price_d1" value="0.00" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'
				</span>
			</td>
		  </tr>

		  <tr>
			  <td style="width:150px">'.$this->l('Impact on price D2:').'</td>
			  <td colspan="2" style="padding-bottom:5px;">
				<select name="attribute_price_impact_d2" id="attribute_price_impact_d2" style="width: 140px;" onchange="check_impact();">
				  <option value="0">'.$this->l('None').'</option>
				  <option value="1">'.$this->l('Increase').'</option>
				  <option value="-1">'.$this->l('Reduction').'</option>
				</select> <sup>*</sup>
				<span id="span_impact">&nbsp;&nbsp;'.$this->l('of').'&nbsp;&nbsp;'.($currency->format == 1 ? $currency->sign.' ' : '').'
					<input type="text" size="6" name="attribute_price_d2" id="attribute_price_d2" value="0.00" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'
				</span>
			</td>
		  </tr>

		  <tr>
			  <td style="width:150px">'.$this->l('Impact on price D3:').'</td>
			  <td colspan="2" style="padding-bottom:5px;">
				<select name="attribute_price_impact_d3" id="attribute_price_impact_d3" style="width: 140px;" onchange="check_impact();">
				  <option value="0">'.$this->l('None').'</option>
				  <option value="1">'.$this->l('Increase').'</option>
				  <option value="-1">'.$this->l('Reduction').'</option>
				</select> <sup>*</sup>
				<span id="span_impact">&nbsp;&nbsp;'.$this->l('of').'&nbsp;&nbsp;'.($currency->format == 1 ? $currency->sign.' ' : '').'
					<input type="text" size="6" name="attribute_price_d3" id="attribute_price_d3" value="0.00" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'
				</span>
			</td>
		  </tr>
				...
				if (is_array($combinaisons))
				{
					...
					foreach ($combinaisons AS $k => $combinaison)
					{
						...
						$combArray[$combinaison['id_product_attribute']]['price'] = $combinaison['price'];
                                                $combArray[$combinaison['id_product_attribute']]['price_D1'] = $combinaison['price_D1'];
                                                $combArray[$combinaison['id_product_attribute']]['price_D2'] = $combinaison['price_D2'];
                                                $combArray[$combinaison['id_product_attribute']]['price_D3'] = $combinaison['price_D3'];
				...
				if (isset($combArray))
					foreach ($combArray AS $id_product_attribute => $product_attribute)
					{
						...
						<tr'.($irow++ % 2 ? ' class="alt_row"' : '').($product_attribute['default_on'] ? ' style="background-color:#D1EAEF"' : '').'>
							...
							onclick="javascript:fillCombinaison(\''.$product_attribute['wholesale_price'].'\', \''.$product_attribute['price'].'\', \''.$product_attribute['price_D1'].'\', \''.$product_attribute['price_D2'].'\', \''.$product_attribute['price_D3'].'\', \''.$product_attribute['weight'].'\', \''.$product_attribute['reference'].'\', \''.$product_attribute['supplier_reference'].'\', \''.$product_attribute['ean13'].'\',
