		...
		if (sizeof($addresses))
		{
			...
			foreach ($addresses AS $address)
				...
				<tr '.($irow++ % 2 ? 'class="alt_row"' : '').'>
					...
					<td>'.($address['ic'] ? $address['ic'] : '--').'</td>
					<td>'.($address['dic'] ? $address['dic'] : '--').'</td>
					...
					<table cellspacing="0" cellpadding="0" class="table" style="width: 29.5em;">
						...
						foreach ($groups as $group)
						{
							...
							<tr class="'.($irow++ % 2 ? 'alt_row' : '').'">
								<td>'.($group['id_group'] != 0 ? '<input type="checkbox" name="groupBox[]" class="groupBox" id="groupBox_'.$group['id_group'].'" value="'.$group['id_group'].'" '.(in_array($group['id_group'], $customer_groups) ? 'checked="checked" ' : '').'/>' : '').'</td>
