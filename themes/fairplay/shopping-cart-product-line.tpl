	...
	<td class="cart_unit"><span class="price">{if $product.sizeh>0}{if $priceDisplay}{convertPrice price=$product.price*100}{else}{convertPrice price=$product.price_wt*100}{/if}{else}{if $priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_wt}{/if}{/if}</span></td>
    ...
    <div  style="float: left;{if $product.sizeh>0}padding-top:6px;{else} padding-top: 6px; padding-left: 8px;{/if}">{if $product.sizeh>0}{$product.pocet}ks {*({$product.sizew}x{$product.sizeh})*}{else}{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.quantity-$quantityDisplayed}{/if}{/if}</div>

    {if $product.sizew==0 && $product.sizeh==0}		
