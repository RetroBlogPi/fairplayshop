...
{elseif $loginonly}
		<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" id="login_form" class="std" style="width: 100%">
			<fieldset style="height: 17em;">
				<h3>{l s='Log in'}</h3>
				<div style="margin: 2px 12px 15px 30px;" >
				  {l s='Account registered on this email already exists. You need to log in to access it.'}
				  {l s='If you do not remember your password, please click: '}<a href="{$base_dir}password.php">{l s='Forgot your password?'}</a>
				</div>
				<div style="margin: 2px 2px 10px 35px;" >
					<label for="email" style="width: 115px; float: left; ">{l s='E-mail address'}</label>
				...
				<div style="margin: 2px 2px 10px 35px;" >
					<label for="passwd" style="width: 115px; float: left;">{l s='Password'}</label>
...
{else}
	{if !isset($email_create)}
                {*
                <form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" id="express-checkout_form" class="std">
                        <fieldset>
                                <h3>{l s='Express checkout'}</h3>
                                <p style="text-align: center"><b>{l s='Fast checkout, no registration required.'}</b></p>
                                <p style="text-align: center"><b>{l s='Just put items into your shopping cart and click:'}</b></p>
                                <br />
                                <center><a href="order.php?step=1" class="button_large">{l s='Proceed to checkout'}</a></center>
                        </fieldset>
                </form>
                *}
		...
		<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" id="create-account_form" class="std">
			<fieldset>
				...
				<h3>{l s='Enter your e-mail address to create your account'}.</h3>
				<p class="text">
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}" method="post" id="account-creation_form" class="std">
		...
		<fieldset class="account_creation">
			...
			<p class="text required">
				...
				<input type="text" class="text" id="ic" name="ic" value="{if isset($smarty.post.ic)}{$smarty.post.ic}{/if}" />
				<sup>*</sup>
			</p>
			<p class="text">
				<label for="dic">{l s='DIC'}</label>
				<input type="text" class="text" id="dic" name="dic" value="{if isset($smarty.post.dic)}{$smarty.post.dic}{/if}" />
			</p>
			</div>				
			<p class="required text">
