...
<h3>{if isset($id_address)}{l s='Modify the address'} {if isset($smarty.post.alias)}"{$smarty.post.alias}"{elseif $address->alias}"{$address->alias|escape:'htmlall':'UTF-8'|regex_replace:"/^(inv-)|(dlv-)/":""}"{/if}{else}{l s='To add a new address, please fill out the form below.'}{/if}</h3>
...
<form action="{$base_dir_ssl}address.php" method="post" class="std">
	<fieldset>
		...
		<p class="radio required">
			...
			<input type="radio" name="id_type" id="id_type2" value="1" {if (isset($smarty.post.id_type) && $smarty.post.id_type == 1) || $address->id_type == 1}checked="checked"{/if} />
		...
		<p class="text required">
			<label for="ic">{l s='IC'}</label>
			<input type="text" id="ic" name="ic" value="{if isset($smarty.post.ic)}{$smarty.post.ic}{else}{$address->ic|escape:'htmlall':'UTF-8'}{/if}" />
			<sup>*</sup>
		</p>
		<p class="text">
			<label for="dic">{l s='DIC'}</label>
			<input type="text" id="dic" name="dic" value="{if isset($smarty.post.dic)}{$smarty.post.dic}{else}{$address->dic|escape:'htmlall':'UTF-8'}{/if}" />
		</p>	
    </div>			
		...
		*}		
		...
		<p class="required text" id="adress_alias">
			...
			<input type="text" id="alias" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{elseif $address->alias}{$address->alias|escape:'htmlall':'UTF-8'|regex_replace:"/^(inv-)|(dlv-)/":""}{elseif isset($select_address)}{else}{l s='My address'}{/if}" />
			<input type="hidden" id="former_alias" name="former_alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{elseif $address->alias}{$address->alias|escape:'htmlall':'UTF-8'}{elseif isset($select_address)}{else}{l s='My address'}{/if}" />
			<sup>*</sup>
		</p>
	</fieldset>
	<p class="submit2">
