		...
		<li class="ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{else}item{/if} {if $smarty.foreach.products.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 1}first_item_of_line{/if} {if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - ($smarty.foreach.products.total % $nbItemsPerLine))}last_line{/if}">
          ...
          <a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.legend|escape:'htmlall':'UTF-8'}">{$product.name|truncate:35:'...'|escape:'htmlall':'UTF-8'}</a>
			...
			<div class="right_block">
				...
				{/if}
				{if !$priceDisplay || $priceDisplay == 2}<div><span class="price" style="display: inline;">{if $product.customsize==1}{convertPrice price=$product.price*100}{else}{convertPrice price=$product.price}{/if}</span>{if $priceDisplay == 2} {l s='+Tx'}{/if}</div>{/if}
				{if $priceDisplay}<div><span class="price" style="display: inline;">{if $product.customsize==1}{convertPrice price=$product.price_tax_exc*100}{else}{convertPrice price=$product.price_tax_exc}{/if}</span>{if $priceDisplay == 2} {l s='-Tx'}{/if}</div>{/if}
