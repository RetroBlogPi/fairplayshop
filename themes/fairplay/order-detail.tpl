...
<ul class="address item">
	...
	{if $address_invoice->ic}<li class="address_ic">{$address_invoice->ic|escape:'htmlall':'UTF-8'}</li>{/if}
	{if $address_invoice->dic}<li class="address_dic">{$address_invoice->dic|escape:'htmlall':'UTF-8'}</li>{/if}
...
<ul class="address alternate_item">
	...
	{if $address_delivery->ic}<li class="address_ic">{$address_delivery->ic|escape:'htmlall':'UTF-8'}</li>{/if}
	{if $address_delivery->dic}<li class="address_dic">{$address_delivery->dic|escape:'htmlall':'UTF-8'}</li>{/if}
	...
	<table class="std">
		<thead>
			<tr>
				...
				<th class="item">{*l s='Unit price'*}Cena za kus/m2/bm</th>
			...
			{if !$product.deleted}
				...
				{if isset($customizedDatas.$productId.$productAttributeId)}
					<tr class="item">
						...
						{*custom size*}
						<td><input class="order_qte_input" name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$customizationQuantityTotal|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{if $product.sizeh>0}{$product.pocet}{* x {$product.sizew*$product.sizeh|round:3} m2*}{else}{$product.customizationQuantityTotal|intval}{/if}</span></label></td>
						<td><label for="cb_{$product.id_order_detail|intval}">{if $product.sizeh>0}{convertPriceWithCurrency price=$product.product_price_wt*100 currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}{/if}</label></td>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td><label for="cb_{$product.id_order_detail|intval}">{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}</label></td>*}
            {*custom size*}
						<td><input class="order_qte_input" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{if $product.sizeh>0}{$product.pocet}{* x {$product.sizew*$product.sizeh|round:3} m2*}{else}{$productQuantity|intval}{/if}</span></label></td>
						<td><label for="cb_{$product.id_order_detail|intval}">{if $product.sizeh>0}{convertPriceWithCurrency price=$product.product_price_wt*100 currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}{/if}</label></td>
...
<div class="table_block">
	<table class="detail_step_by_step std">
		<thead>
			<tr>
				<th class="first_item" style="width:190px;">{l s='From'}</th>
