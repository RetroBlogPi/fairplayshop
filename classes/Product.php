...
class		Product extends ObjectModel
{
	...
	// added support for wholesale prices
	public    $price_D1 = 0;
	public    $price_D2 = 0;
	public    $price_D3 = 0;
	//-----------------------------------
  ...
  /** @var boolean customsize */
	...
	public function getFields()
	{
		...
		// added support for wholesale prices ----------
		$fields['price_D1'] = floatval($this->price_D1);
		$fields['price_D2'] = floatval($this->price_D2);
		$fields['price_D3'] = floatval($this->price_D3);
		//----------------------------------------------
		...
		$fields['customsize'] = intval($this->customsize);
	...
	*/
	public function updateProductAttribute($id_product_attribute, $wholesale_price, $price, $price_D1, $price_D2, $price_D3, $weight, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $location = NULL)
	{
		Db::getInstance()->Execute('
		...
		'price' => floatval($price),
                'price_D1' => floatval($price_D1),
                'price_D2' => floatval($price_D2),
                'price_D3' => floatval($price_D3),
	...
	*/
	...
	public static function getPriceStatic($id_product, $usetax = true, $id_product_attribute = NULL, $decimals = 6, $divisor = NULL, $only_reduc = false, $usereduc = true, $quantity = 1, $forceAssociatedTax = false, $id_customer = NULL, $id_cart = NULL, $id_address_delivery = NULL)
	{
		...
		$wholesaleCustCategory = Customer::getWholesaleCategory($id_customer);


		// Caching system
		$cacheId = $id_product.'-'.($usetax?'1':'0').'-'.$id_product_attribute.'-'.$decimals.'-'.$divisor.'-'.($only_reduc?'1':'0').'-'.($usereduc?'1':'0').'-'.$quantity.'-'.$wholesaleCustCategory;
		if (isset(self::$_prices[$cacheId]))
			return self::$_prices[$cacheId];
		...
		elseif($wholesaleCustCategory == 1) {
		    $sql_price = 'pa.`price_D1`';
		}
		elseif($wholesaleCustCategory == 2) {
		    $sql_price = 'pa.`price_D2`';
		}
		else {
		    $sql_price = 'pa.`price_D3`';
		}


		// Getting price
		$result = Db::getInstance()->getRow('
		SELECT p.`price`, p.`price_D1` , p.`price_D2`, p.`price_D3`,
		p.`reduction_price`, p.`reduction_percent`, p.`reduction_from`, p.`reduction_to`, p.`id_tax`, t.`rate`,
		'.($id_product_attribute ? $sql_price : 'IFNULL((SELECT pa.price FROM `'._DB_PREFIX_.'product_attribute` pa WHERE id_product = '.intval($id_product).' AND default_on = 1), 0)').' AS attribute_price
		FROM `'._DB_PREFIX_.'product` p
		'.($id_product_attribute ? 'LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa ON pa.`id_product_attribute` = '.intval($id_product_attribute) : '').'
		LEFT JOIN `'._DB_PREFIX_.'tax` AS t ON t.`id_tax` = p.`id_tax`
		WHERE p.`id_product` = '.intval($id_product));
	    ...
	    if ($wholesaleCustCategory == 1)
	    {
	        //$wholeSalePrice = Tools::convertPrice(floatval($result['price_D1']));
	        $wholeSalePrice = floatval($result['price_D1']);
	    }
	    elseif ($wholesaleCustCategory == 2)
	    {
	        //$wholeSalePrice = Tools::convertPrice(floatval($result['price_D2']));
	        $wholeSalePrice = floatval($result['price_D2']);
	    }
	    elseif ($wholesaleCustCategory == 3)
	    {
	        //$wholeSalePrice = Tools::convertPrice(floatval($result['price_D3']));
	        $wholeSalePrice = floatval($result['price_D3']);
	    }
		...
		// Attribute price
		$attribute_price = array_key_exists('attribute_price', $result) ? floatval($result['attribute_price']) : 0;
		$attribute_price = $usetax ? round($attribute_price, 2) : ($attribute_price / (1 + (($tax ? $tax : $result['rate']) / 100)));
	...
	// added for wholesale support (find customer's wholesale category)
	public static function getWholesaleCategory($id_customer = NULL)
	{
	    global $cookie;
		// Get id_customer if exists
		if (!$id_customer)
  ...
  /*
	...
	*/
	...
	public function getPriceWithoutReduct($notax = false)
	{
	    ...
	    // added columns for wholesale prices
		$res = Db::getInstance()->getRow('
			SELECT p.`price`,
			p.`price_D1`,
			p.`price_D2`,
		...
		$wholesaleCustCategory = self::getWholesaleCategory();
		$taxPrice = 0;
		$woTaxPrice = 0;
		$tax = floatval(Tax::getApplicableTax(intval($res['id_tax']), floatval($res['rate'])));
		...
		if ($wholesaleCustCategory == 0)
		{
    		$taxPrice = ($res['price']) * (1 + $tax / 100);
    		$woTaxPrice = ($res['price']);
		}
		elseif($wholesaleCustCategory == 1)
		{
    		$taxPrice = ($res['price_D1']) * (1 + $tax / 100);
    		$woTaxPrice = ($res['price_D1']);
		}
		elseif($wholesaleCustCategory == 2)
		{
    		$taxPrice = ($res['price_D2']) * (1 + $tax / 100);
    		$woTaxPrice = ($res['price_D2']);
		}
		else
		{
    		$taxPrice = ($res['price_D3']) * (1 + $tax / 100);
    		$woTaxPrice = ($res['price_D3']);
		}
	...
	*/
	public function getAttributesGroups($id_lang)
	{
	  ...
	  $wholesaleCustCategory = self::getWholesaleCategory();
                ...
                if ($wholesaleCustCategory == 0) {
    		$sql_price = 'pa.`price` AS price';
		}
		elseif($wholesaleCustCategory == 1) {
		    $sql_price = 'pa.`price_D1` AS price';
		}
		elseif($wholesaleCustCategory == 2) {
		    $sql_price = 'pa.`price_D2` AS price';
		}
		else {
		    $sql_price = 'pa.`price_D3` AS price';
		}
		...
		a.`color` AS attribute_color, pa.`id_product_attribute`, pa.`quantity`, '.$sql_price.', pa.`ecotax`, pa.`weight`, pa.`default_on`, pa.`reference`
		FROM `'._DB_PREFIX_.'product_attribute` pa
		LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute` a ON a.`id_attribute` = pac.`id_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
