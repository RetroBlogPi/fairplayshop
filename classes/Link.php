...
class Link
{
	...
	public function getProductLink($id_product, $alias = NULL, $category = NULL, $ean13 = NULL)
	{
		...
		if (is_object($id_product))
			return ($this->allow == 1)?(_PS_BASE_URL_.__PS_BASE_URI__.(($id_product->category != 'home' AND !empty($id_product->category)) ? $id_product->category.'/' : '').intval($id_product->id).'-'.$id_product->link_rewrite.($id_product->ean13 ? '' : '').'.html') : 
