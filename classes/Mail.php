...
class Mail
{
	static public function Send($id_lang, $template, $subject, $templateVars, $to, $toName = NULL, $from = NULL, $fromName = NULL, $fileAttachment = NULL, $modeSMTP = NULL, $templatePath = _PS_MAIL_DIR_)
	{
		...
		# do not send email, if it was only generated
		$optional_email = Configuration::get('OPC_DISPLAY_OPTIONAL_EMAIL');
		$hide_email = Configuration::get('OPC_DISPLAY_HIDE_EMAIL');
		$generated_domain = Configuration::get('OPC_FIELD_GEN_DOMAIN');

		$dont_send = false;

		if ($hide_email || $optional_email) {
			if (is_array($to))
			{
				foreach ($to as $key => $addr)
				  if (preg_match('/@'.$generated_domain.'/', $addr));
				    $dont_send = true;
			} else {
				if (preg_match('/@'.$generated_domain.'/', $to))
				    $dont_send = true;
			}
		}
		...
		try {
			...
			$mailPasswdTemplateHtmlPath = $templatePath.$iso.'/mail_passwd.html';
			$mailPasswdTemplateTxtPath = $templatePath.$iso.'/mail_passwd.txt';


			$templateHtml = file_get_contents($templatePath.$template.'.html');
			$templateTxt = file_get_contents($templatePath.$template.'.txt');

			if ($templateVars['{mail_passwd}'] != "")
			{
			  if (file_exists($mailPasswdTemplateHtmlPath))
		  	  {
			    $mailPasswdTemplateHtml = file_get_contents($mailPasswdTemplateHtmlPath);
                            $templateHtml = str_replace("{mail_passwd}", $mailPasswdTemplateHtml, $templateHtml);
			  }
			  if (file_exists($mailPasswdTemplateTxtPath))
		  	  {
			    $mailPasswdTemplateTxt = file_get_contents($mailPasswdTemplateTxtPath);
                            $templateTxt = str_replace("{mail_passwd}", $mailPasswdTemplateTxt, $templateTxt);
			  }
			}


			$templateTxt = strip_tags(html_entity_decode($templateTxt, NULL, 'utf-8'));
