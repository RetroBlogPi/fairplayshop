...
class		Cart extends ObjectModel
{
	...
	public function getProducts($refresh = false, $id_product = false)
	{
		...
		$sql = '
		SELECT cp.`id_product_attribute`, cp.`sizew`, cp.`pocet`, cp.`sizeh`, cp.`id_product`, cp.`quantity` AS cart_quantity, pl.`name`,
  ...
  /*ve.cz*/
	public	function updateQty($quantity, $id_product, $id_product_attribute = NULL, $id_customization = false, $operator = 'up',$sizew=0,$sizeh=0,$pocet=0)
	{
		self::$_nbProducts = 0;
		if (intval($quantity) <= 0)
			return $this->deleteProduct(intval($id_product), intval($id_product_attribute), intval($id_customization));
        ...
        else
        {
          if ($operator == 'up')
				...
				if ($newQty <= 0)
					...
					Db::getInstance()->Execute('
					UPDATE `'._DB_PREFIX_.'cart_product`
					SET `quantity` = '.$qty.', `sizew`='.$sizew.', `sizeh`='.$sizeh.'
					'.($operator == 'up' ? ', `date_add` = \''.pSql(date('Y-m-d H:i:s')).'\'' : '').'
					WHERE `id_product` = '.intval($id_product).
					($id_product_attribute != NULL ? ' AND `id_product_attribute` = '.intval($id_product_attribute) : '').'
					AND `id_cart` = '.intval($this->id));
			...
			else
			{
				...
				if (!Db::getInstance()->AutoExecute(_DB_PREFIX_.'cart_product', array('id_product' => intval($id_product),
				'id_product_attribute' => intval($id_product_attribute), 'id_cart' => intval($this->id),
				'quantity' => intval($quantity), 'sizew' => $sizew, 'sizeh' => $sizeh, 'pocet'=>$pocet, 'date_add' => pSql(date('Y-m-d H:i:s'))), 'INSERT'))
					return false;
			}
		}
		return $this->_updateCustomizationQuantity(intval($quantity), intval($id_customization), intval($id_product), intval($id_product_attribute), $operator);
	...
	*/
    ...
    /**
     *  Select default (if available) or then cheapest carrier currently allowed for default zone
     */

    function getSuitableCarrier($id_zone)
    {
	//echo "getting suitable carrier";
	...
	$pickup_carrier = Configuration::get('OPC_FIELD_PICKUP_CRR');
        ...
        foreach ($result AS $k => $row)
        {
                ...
                $row['price'] = $this->_getOrderShippingCost($id_zone, intval($row['id_carrier']), true);
                $row['price_tax_exc'] = $this->_getOrderShippingCost($id_zone, intval($row['id_carrier']), false);
	...
	//print_r($resultsArray);
    	if ($cookie_carrier_present)
          $result = $cookie_carrier;
	elseif (Configuration::get('OPC_DISPLAY_CHEAP_CARRIER'))
          $result = $cheapest_carrier;
	elseif ($default_carrier_present)
          $result = $default_carrier;

	/*echo "<br />cookie_carrier: $cookie_carrier<br />";
	echo "cheapest_carrier: $cheapest_carrier<br />";
	echo "default_carrier: $default_carrier<br />";
	echo "pickup_carrier: $pickup_carrier<br />";
	echo "chosen : $result"; exit;*/
	...
	*/
    function getOrderShippingCost($id_carrier = NULL, $useTax = true, $artifical_zone_id = 0) {
  ...
  */
  ...
  if (isset($configuration['PS_SHIPPING_FREE_WEIGHT']) AND $this->getTotalWeight() >= floatval($configuration['PS_SHIPPING_FREE_WEIGHT']) AND floatval($configuration['PS_SHIPPING_FREE_WEIGHT']) > 0)    
	...
	// cart reset (when all products are deleted)
/*	if ($this->id_carrier == 0) {
		unset($cookie->userChoiceCarrier);
	//	unset($cookie->artificial_zone_id); // problem in gChrome
	}
*/

        if (isset($artifical_zone_id) AND $artifical_zone_id > 0)
		$id_zone = $artifical_zone_id;
        elseif (isset($cookie->artificial_zone_id) AND $cookie->artificial_zone_id > 0)
                $id_zone = $cookie->artificial_zone_id;
        elseif (isset($this->id_address_delivery) AND $this->id_address_delivery)
		$id_zone = Address::getZoneById(intval($this->id_address_delivery));
	else
		$id_zone = intval($defaultCountry->id_zone);

	if ($id_carrier)
	  //return $this->_getOrderShippingCost($id_carrier, $useTax);
	  //return 100+intval($id_carrier);
	  return $this->_getOrderShippingCost($id_zone, intval($id_carrier), $useTax);
	elseif ($this->id_carrier)
	  //return 0+intval($this->id_carrier);
	  return $this->_getOrderShippingCost($id_zone, intval($this->id_carrier), $useTax);
	else
	  //return $this->getSuitableCarrier($id_zone);
	  //return $this->_getOrderShippingCost();
	  return $this->_getOrderShippingCost($id_zone, $this->getSuitableCarrier($id_zone), $useTax);
    }

  function _getOrderShippingCost($id_zone, $id_carrier = NULL, $useTax = true)
  {
		global $defaultCountry;

		// VE.cz - pridani tohoto radku. $orderTotal tady nebylo vubec definovano!!!
		$orderTotal = $this->getOrderTotal(true, 7);
	...
	*/
	function getSummaryDetails()
	{
		global $cookie;
		return array(
                        'delivery' => new Address(intval($this->id_address_delivery), $cookie->id_lang),
                        'invoice' => new Address(intval($this->id_address_invoice), $cookie->id_lang),
  ...
  /* DEPRECATED */
	...
	/* VE.cz */
	public function solveQuantity($w,$h,$pocet,$pid) // výpočet v metrech - vrací převedené na celé dm2
	{
    if($w>0) return ceil($w*$h*100*$pocet); // m2
    else return ($h*100*$pocet); // bm
  }
}