...
public function getLastConnections()
	...
	*/	
	...
	{
		Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'customer_group` WHERE `id_customer` = '.intval($this->id).' AND `id_group` != 0');
	}
	...
	// added for wholesale prices support --------------
	public function getGroupsWODefaultCat()
	{
		$groups = array();
		$result = Db::getInstance()->ExecuteS('
		SELECT cg.`id_group`
		FROM '._DB_PREFIX_.'customer_group cg
		WHERE cg.`id_customer` = '.intval($this->id).' AND cg.`id_group` != 1');
		if (!$result)
		{
    		$result = Db::getInstance()->ExecuteS('
    		SELECT cg.`id_group`
  ...
  //-----------------------------------------------------------------------------------------------------------------------------	
	...
	//added for wholesale prices support
	public static function getWholesaleCategory($id_customer)
	{
		// strings must be equal to category names !!!
	    $D1 = "Skupina D1";
	    $D2 = "Skupina D2";
	    $D3 = "Skupina D3";
		//--------------------------------------------
