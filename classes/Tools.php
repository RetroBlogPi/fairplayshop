...
class Tools
{
	...
	*/                    
	static public function displayPrice($price, $currency, $no_utf8 = false, $convert = true, $decimals = 0)
	{
    ...
    if ($decimals > 0) $c_decimals = $decimals;
		else $c_decimals = (is_array($currency) ? intval($currency['decimals']) : intval($currency->decimals)) * _PS_PRICE_DISPLAY_PRECISION_;
	...
	*/
	...
	static public function displayDateFa($date, $id_lang, $full = false, $separator='-')
	{
	 	if (!$date OR !strtotime($date))
	 		return $date;
		if (!Validate::isDate($date) OR !Validate::isBool($full))
	 	...
	 	$dat = mktime(0,0,0,$tmpTab[1],$tmpTab[2],$tmpTab[0]);

	 	return date("j.n.Y",$dat);
	}
	
	static public function displayDateSplatno($date, $id_lang,$splatna=14, $full = false, $separator='-')
	{
	 	if (!$date OR !strtotime($date))
	 		return $date;
		if (!Validate::isDate($date) OR !Validate::isBool($full))
	 	...
	 	$dat = mktime(0,0,0,$tmpTab[1],$tmpTab[2],$tmpTab[0]);
	 	$splatno = $dat + $splatna*86400;

	 	return date("j.n.Y",$splatno);
	}
