...
document.getElementsByName = function(name){
 ...
 function updateTotals(total_delivery_id, total_price_id, carrier_price_id) {
	...
	//alert(newTotalShipping);
	actualTotalPrice =  parseFloat(total_price_uniform.replace(/[^\d,.]/g,''));	

	newTotalShippingStr = newTotalShipping.toFixed(0);
	//alert(newTotalShippingStr);
	newTotalPrice = (actualTotalPrice - actualTotalShipping + newTotalShipping);
	newTotalPriceStr = newTotalPrice.toFixed(0);
...
//				alert('[deliver_price_unifomr]actual/newTotalShippingStr = '+delivery_price_uniform+']' +actualTotalShipping +'/'+ newTotalShippingStr);
	...
	$(total_delivery_id).html(newTotalShippingStr + ' Kč');// + ' &euro;');
	
	if (!isNaN(actualTotalShipping)) 
	$(total_price_id).html(newTotalPriceStr + ' Kč');// + ' &euro;');
}


  function set_carrier2(price,obj)
  {
    ...
    updateTotals('#cart_block_shipping_cost', '#cart_block_total', '#price_carrier_hidden');
    
    var p=parseFloat($("#order-detail-content tr.cart_total_delivery td.price").html());
    p=p+price;
    $("#order-detail-content tr.cart_total_delivery td.price").html(p.toFixed(0)+' Kč');
    //kosik vpravo
    $("#cart_block_shipping_cost").html(p.toFixed(0)+' Kč');
    
    var tp=parseFloat($("#order-detail-content tr.cart_total_price td.price").html());
    tp=tp+price;
    $("#order-detail-content tr.cart_total_price td.price").html(tp.toFixed(0)+' Kč');
    //kosik vpravo
    $("#cart_block_total").html(tp.toFixed(0)+' Kč');
     
    $.ajax({
      type: 'GET',
      url: 'order_cookie.php',
      cache: false,
      async: true,
      data: 'a=1&inputid='+obj.value,
      dataType: 'json',
      success: function (jsonData)
      {
        $("#pm"+jsonData.inputid).click();
      }
    });
  }

 function set_carrier(carrier_price, manual, free, logged) {
...
//				alert('success');
		...
		//udrzeni typu platby
		setTimeout('selectPaymentInput()',1500);
	...
	updateZonesCarriers : function(zoneId){
		...
		$.ajax({
			...
			success: function(jsonData)
			{
				...
				$('#carriers2').slideUp(400, function() {$('#carriers_wait_img').fadeOut(500); $('#carriers2').html(jsonData.html_data).slideDown(500).fadeTo(400, 1); set_carrier(jsonData.selected_price, 1, jsonData.free_carrier, jsonData.is_logged); updatePaymentIfShip2payInactive()});
			},
			error: function(XMLHttpRequest, textStatus, errorThrown) {
			}
		});
...
function selectPaymentInput()
{
    //$("#pm"+$.cookie("paymenttype")).click();
    /*
    $.ajax({
      type: 'GET',
      url: 'order_cookie.php',
