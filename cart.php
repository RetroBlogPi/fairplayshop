...
if ($add OR Tools::getIsset('update') OR $delete)
{
  ...
  $qty = intval(abs(Tools::getValue('qty',1)));  
	...
	//VE.cz custom size
	$sizew_id = Tools::getValue('group_4');
	if($sizew_id) {
  	$tmp=Db::getInstance()->executeS('SELECT `name` FROM '._DB_PREFIX_.'attribute_lang WHERE id_lang = '.$cookie->id_lang.' AND id_attribute='.intval($sizew_id));
  	$sizew=$tmp[0]['name'];
  } else $sizew=0;
	
	$sizeh_id = Tools::getValue('group_5');
	if($sizeh_id) {
    $tmp=Db::getInstance()->executeS('SELECT `name` FROM '._DB_PREFIX_.'attribute_lang WHERE id_lang = '.$cookie->id_lang.' AND id_attribute='.intval($sizeh_id));
  	$sizeh=$tmp[0]['name'];
  } else $sizeh=0;
	
	$pocet=Tools::getValue('pocet');
	$customsize = Tools::getValue('customsize');
	$sizew=(str_replace(",",".",$sizew)/1000);
	$sizeh=(str_replace(",",".",$sizeh)/1000);

	if($sizeh)
	{
	  if($sizeh>0) {
      $qty=Cart::solveQuantity($sizew,$sizeh,$pocet,$idProduct); // vrátí potøebnou plochu v dm2 nebo délku v bm (bìžný metr)
      if($qty==0) $errors[] = Tools::displayError('cannot solve valid size');
    } else {
      $errors[] = Tools::displayError('invalid size');
      $qty=0;
    }
  } else {
    if($customsize==1) $errors[] = Tools::displayError('null quantity');
  }  
		...
		else
		{
			...
			if (!sizeof($errors))
			{
				if ($add AND $qty >= 0)
				{
					...
					if (!sizeof($errors) AND !$cart->updateQty(intval($qty), intval($idProduct), intval($idProductAttribute), $customizationId, Tools::getValue('op', 'up'),$sizew,$sizeh,$pocet))
...
//VE.cz - size
if(Tools::getValue('ajax')==1 && Tools::getValue('ctp')==1)
{
  $sizew = Tools::getValue('sizew');
	$sizeh = Tools::getValue('sizeh');
	$pocet=Tools::getValue('pocet');
	$sizew=str_replace(",",".",$sizew)/1000;
	$sizeh=str_replace(",",".",$sizeh)/1000;
  ...
  if($sizeh>0 && $pocet) {
    $qty=Cart::solveQuantity($sizew,$sizeh,$pocet,$pid); // vrátí potøebnou plochu v dm2 nebo v bm
    $unitPrice=Product::getPriceStatic($pid);
    if($sizew>0) $arr=array("qty"=>$qty,"s"=>round($sizew*$sizeh*$pocet,2),"price"=>round($qty*$unitPrice));
    else $arr=array("qty"=>$qty,"s"=>round($sizeh*$pocet,2),"price"=>round($qty*$unitPrice));
    if($qty==0) echo "{}";
