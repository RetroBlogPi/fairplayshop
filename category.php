...
$smarty->assign(array(
  ...
  //VE.cz - subcats fairplay
  $parents=$category->getParentsCategories(intval($cookie->id_lang));
  if(isset($parents[0])) {
    $tmp=Category::getChildren($parents[0]['id_category'], intval($cookie->id_lang));
    if(isset($parents[1])) {
      $subcats1=Category::getChildren($parents[1]['id_category'], intval($cookie->id_lang));
      $subcats2=$tmp;
    }
    else {
      $subcats1=$tmp;
